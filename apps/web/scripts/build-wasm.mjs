...
// `crates/weather-alerts` (S06), `crates/forecast-web` (S08 follow-up), and
// `crates/mrms-web` (S09 Phase 3) -- and generates each one's `wasm-bindgen`
...
//
// All four crates' `wasm-bindgen --target web` output is written to the
// same `apps/web/src/wasm/` directory (distinguished by `--out-name`, so
// `radar_web.*`, `weather_alerts.*`, `forecast_web.*`, and `mrms_web.*` sit
// side by side with no collision) rather than separate directories -- there
// is no meaningful benefit to splitting them, and every existing import path
// in `src/radar/wasmModule.ts` (`../wasm/radar_web.js`) keeps working
...
// Cargo.toml, crates/forecast-web/Cargo.toml, crates/mrms-web/Cargo.toml,
// and this app's README.md):
...
buildWasmCrate("mrms-web", "mrms_web");

console.log(
  `[build-wasm] wrote wasm-bindgen output for radar-web, weather-alerts, forecast-web, and mrms-web to ${outDir}`,
);
